...
ARG SNAPCTL_VERSION="v1.0.0"
ARG SNAPCTL_ARCH="aarch64-unknown-linux-musl"
...
RUN apk add --update --no-cache curl && \
    curl \
        ...
        --location \
        --fail \
        --request GET "https://github.com/open-sori/snapctl/releases/download/${SNAPCTL_VERSION}/snapctl-${SNAPCTL_ARCH}" \
        --output /tmp/snapctl && \
    chmod +x /tmp/snapctl
